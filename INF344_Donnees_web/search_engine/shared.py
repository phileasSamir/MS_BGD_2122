...
def extractListOfWords(content):
    ...
    # this regex tries to find words that should not contain any weird chars.
    regexAlphanum=re.compile(r"\w+")
    regexNum=re.compile(r"\d") # this captures digits
    for t in textTags:
        if t.parent.name not in blacklist: # we don't want to index thoses texts
            for e in regexAlphanum.findall(t): # we want words without weird chars
                if not regexNum.search(t): # we don't want digits
                    yield e
